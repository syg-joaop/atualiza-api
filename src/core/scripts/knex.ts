...
import { IClienteDb, IVersaoEsperadaCliente } from "../interfaces/cliente.interface";
...
export class KnexConnection {
    ...
    public async conexaoCliente(idEmpresa: number) {
        try {
            ...
            const clienteDb = await knex("conexoes_clientes_cloud").where({ idempresa: idEmpresa }).first();
            const { user, port, database, password, host } = clienteDb
    ...
    public async makeMigration(migration: IMigration) {
        try {
            ...
            const knex = await this.conexao();
            let migrate = await knex.migrate.up({
                name: name,
                directory: directory,
            });
